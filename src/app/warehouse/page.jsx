...
import { getAllDashbordOrders, handleAssignPicker, isPickerAssigningError } from "@/services/features/orderSlice";
      ...
      <div className="warehouse-tab">
        {
          dashboardOrders.result?.map((order) => {
            return (
              <OrderCard action_label="Assign" open_picker_modal={(orderId) => setPicker(orderId)} open_modal={(data) => setModalOpen(data)} order_dat={order} key={order.id} />
            )
          })
        }
